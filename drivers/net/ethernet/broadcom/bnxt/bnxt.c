...
static int bnxt_init_one_rx_ring(struct bnxt *bp,
				 struct bnxt_rx_ring_info *rxr)
{
	struct bnxt_ring_struct *ring;
	...
	ring = &rxr->rx_ring_struct;
	netif_queue_set_napi(bp->dev, ring->grp_idx,
			     NETDEV_QUEUE_TYPE_RX, &rxr->bnapi->napi);
...
static int bnxt_init_rx_rings(struct bnxt *bp)
{
	...
	for (i = 0; i < bp->rx_nr_rings; i++) {
		rc = bnxt_init_one_rx_ring(bp, &bp->rx_ring[i]);
		if (rc)
			break;
	}
				   ...
				   bool close_path)
{
	...
	//printk("----- bnxt_hwrm_rx_ring_free: grp_idx=%u, fw_ring_id=%u, cmpl_id=%d\n", grp_idx, ring->fw_ring_id, close_path ? cmpl_ring_id : INVALID_HW_RING_ID);
...
static int bnxt_queue_mem_alloc(struct net_device *dev, void *qmem, int idx)
{
	...
	rc = bnxt_alloc_rx_page_pool(bp, clone, rxr->page_pool->p.nid, idx);
	if (rc)
		return rc;

	ring = &clone->rx_ring_struct;
	ring->ring_mem.pg_tbl = NULL;
	ring->ring_mem.pg_tbl_map = 0;
	rc = bnxt_alloc_ring(bp, &ring->ring_mem);
	if (rc)
...
static int bnxt_queue_start(struct net_device *dev, void *qmem, int idx)
{
	...
	struct bnxt_ring_struct *ring;
	struct bnxt_ring_mem_info *rmem;
	struct bnxt_ring_struct *clone_ring;
	struct bnxt_ring_mem_info *clone_rmem;
	...
	WARN_ON(rxr->page_pool == clone->page_pool);
...
static int bnxt_queue_stop(struct net_device *dev, void *qmem, int idx)
{
	...
	/* rebuild struct bnxt_ring_structs */
	__bnxt_init_rx_ring_struct(bp, qmem);
