 ...
 **/
struct fbnic_tlv_msg *fbnic_tlv_msg_alloc(struct fbnic_dev *fbd, u16 msg_id)
{
	struct fbnic_tlv_hdr hdr = { 0 };
	struct fbnic_tlv_msg *msg;

	msg = (struct fbnic_tlv_msg *)fbnic_alloc_page(fbd, GFP_KERNEL);
	if (!msg)
		return NULL;
 ...
 *
 *  Returns NULL on failure to allocate or pointer to new TLV test message.
 **/
struct fbnic_tlv_msg *fbnic_tlv_test_create(struct fbnic_dev *fbd)
{
	...
	msg = fbnic_tlv_msg_alloc(fbd, FBNIC_TLV_MSG_ID_TEST);
	if (!msg)
		return NULL;
				       ...
				       struct fbnic_tlv_msg **results)
{
	...
	get_unsigned_result(FBNIC_TLV_TEST_MSG_U64, str->test_u64);
	get_unsigned_result(FBNIC_TLV_TEST_MSG_U32, str->test_u32);
	get_unsigned_result(FBNIC_TLV_TEST_MSG_U16, str->test_u16);
	get_signed_result(FBNIC_TLV_TEST_MSG_S64, str->test_s64);
	get_signed_result(FBNIC_TLV_TEST_MSG_S32, str->test_s32);
	get_signed_result(FBNIC_TLV_TEST_MSG_S16, str->test_s16);
...
static const struct fbnic_tlv_parser test_tlv_parser[] = {
	FBNIC_TLV_MSG_TEST,
	FBNIC_TLV_MSG_ERROR
};

int fbnic_tlv_self_test(struct fbnic_dev *fbd)
{
	...
	if (fbnic_tlv_msg_parse(NULL, test_msg, test_tlv_parser))
		result = 20;
