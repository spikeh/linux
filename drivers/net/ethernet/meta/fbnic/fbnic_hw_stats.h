...
struct fbnic_rxb_enqueue_stats {
	...
	struct fbnic_stat_counter pause, integrity_err, mac_err;
	struct fbnic_stat_counter parser_err, frm_err;
};

struct fbnic_rxb_fifo_stats {
	struct fbnic_hw_stat drop, trunc;
	struct fbnic_stat_counter trans_pause, trans_drop, trans_ecn;
...
struct fbnic_rpc_stats {
	...
	struct fbnic_stat_counter macda_hit[32], macda_miss;

	struct fbnic_stat_counter ipsrc_hit[8], ipsrc_miss;
	struct fbnic_stat_counter ipdst_hit[8], ipdst_miss;
	struct fbnic_stat_counter outer_ipsrc_hit[8], outer_ipsrc_miss;
	struct fbnic_stat_counter outer_ipdst_hit[8], outer_ipdst_miss;

	struct fbnic_stat_counter tcam_act_hit[64], tcam_act_miss;
};
...
struct fbnic_pcie_stats {
	...
	struct fbnic_stat_counter ib_cpl_tlp, ib_cpl_dword;
...
struct fbnic_fec_stats {
	struct {
		struct fbnic_stat_counter total;
		struct fbnic_stat_counter lanes[FBNIC_RSFEC_MAX_LANES];
	} corrected_blocks, uncorrectable_blocks, SymbolErrorDuringCarrier;
};

#define FBNIC_RMON_HIST_MAX 13
struct fbnic_rmon_stats {
	...
	struct fbnic_stat_counter hist[FBNIC_RMON_HIST_MAX];
	struct fbnic_stat_counter hist_tx[FBNIC_RMON_HIST_MAX];
};
...
struct fbnic_hw_q_stats {
	struct fbnic_hw_stat tde[2];
	struct fbnic_stat_counter tde_pkt_err[2];

	struct fbnic_hw_stat rde;
...
struct fbnic_hw_stats {
	...
	struct fbnic_hw_q_stats hw_q[FBNIC_MAX_QUEUES_ASIC];
};

u64 fbnic_stat_rd64(struct fbnic_dev *fbd, u32 reg, u32 offset);
#define fbnic_stat_rd(_fbd, _reg) fbnic_stat_rd64(_fbd, _reg, 1)
