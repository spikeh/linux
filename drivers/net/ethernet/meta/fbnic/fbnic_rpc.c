...
void fbnic_write_macda(struct fbnic_dev *fbd)
{
	...
	/* Notify firmware of changes to MACDA TCAM */
...
static void fbnic_clear_tce_tcam_entry(struct fbnic_dev *fbd, unsigned int idx)
{
	...
	for (i = 0; i <= FBNIC_TCE_TCAM_WORD_LEN; i++)
		wr32(FBNIC_TCE_RAM_TCAM(idx, i), 0);
}
				      ...
				      struct fbnic_mac_addr *mac_addr)
{
	...
	idx2dest_map = rd32(FBNIC_TCE_TCAM_IDX2DEST_MAP);
	idx2dest_map &= ~(FBNIC_TCE_TCAM_IDX2DEST_MAP_DEST_ID_0 << (4 * idx));
	idx2dest_map |= dest << (4 * idx);

	wr32(FBNIC_TCE_TCAM_IDX2DEST_MAP, idx2dest_map);
}
...
/**
 * fbnic_write_tce_tcam - Write Tx path TCAM to enable host to BMC traffic
 * @fbd: Pointer to fbnic device struct
 *
 * This function is meant to write entries from the RPC MACDA to the Tx
 * TCAM so that we can grap any traffic that may be meant for the BMC and
 * route it there.
 *
 * Note that there isn't any metadata stored about where we have been
 * writing addresses in the TCAM other than the last entry we wrote to, or
 * the starting location for the next set of writes. We start at the
 * opposite end of the TCAM in each pass so that as we either add or delete
 * entries all entries will be available in the TCAM assuming there is no
 * significant reordering of the entries.
 **/
...
static void fbnic_clear_ip_src_entry(struct fbnic_dev *fbd, unsigned int idx)
{
	...
	for (i = 0; i <= FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_IPSRC(idx, i), 0);
}
...
static void fbnic_clear_ip_dst_entry(struct fbnic_dev *fbd, unsigned int idx)
{
	...
	/* invalidate entry and clear addr state info */
	for (i = 0; i <= FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_IPDST(idx, i), 0);
}
					   ...
					   unsigned int idx)
{
	...
	/* invalidate entry and clear addr state info */
	for (i = 0; i <= FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_OUTER_IPSRC(idx, i), 0);
}
					   ...
					   unsigned int idx)
{
	...
	/* invalidate entry and clear addr state info */
	for (i = 0; i <= FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_OUTER_IPDST(idx, i), 0);
}

static void fbnic_clear_ip_addr(struct fbnic_dev *fbd)
{
	int idx;
	...
	for (idx = ARRAY_SIZE(fbd->ip_src); idx--;) {
		...
		if (ip_addr->state != FBNIC_TCAM_S_VALID)
			continue;
		 ...
		 * this tcam the next time fbnic_write_macda is called.
		 */
		ip_addr->state = FBNIC_TCAM_S_UPDATE;
	}
				     ...
				     struct fbnic_ip_addr *ip_addr)
{
	...
	for (i = 0; i < FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_IPSRC(idx, i),
				     ...
				     struct fbnic_ip_addr *ip_addr)
{
	...
	for (i = 0; i < FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_IPDST(idx, i),
		     FIELD_PREP(FBNIC_RPC_TCAM_IP_ADDR_MASK, ntohs(*mask--)) |
		     FIELD_PREP(FBNIC_RPC_TCAM_IP_ADDR_VALUE, ntohs(*value--)));
	wrfl();
					   ...
					   struct fbnic_ip_addr *ip_addr)
{
	...
	for (i = 0; i < FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_OUTER_IPSRC(idx, i),
		     FIELD_PREP(FBNIC_RPC_TCAM_IP_ADDR_MASK, ntohs(*mask--)) |
		     FIELD_PREP(FBNIC_RPC_TCAM_IP_ADDR_VALUE, ntohs(*value--)));
	wrfl();

	wr32(FBNIC_RPC_TCAM_OUTER_IPSRC(idx, i), FBNIC_RPC_TCAM_VALIDATE);
}
					   ...
					   struct fbnic_ip_addr *ip_addr)
{
	...
	for (i = 0; i < FBNIC_RPC_TCAM_IP_ADDR_WORD_LEN; i++)
		wr32(FBNIC_RPC_TCAM_OUTER_IPDST(idx, i),
		     FIELD_PREP(FBNIC_RPC_TCAM_IP_ADDR_MASK, ntohs(*mask--)) |
		     FIELD_PREP(FBNIC_RPC_TCAM_IP_ADDR_VALUE, ntohs(*value--)));
	wrfl();

	wr32(FBNIC_RPC_TCAM_OUTER_IPDST(idx, i), FBNIC_RPC_TCAM_VALIDATE);
}
