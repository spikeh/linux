...
static int fbnic_ptp_adjfine(struct ptp_clock_info *ptp, long scaled_ppm)
{
	...
	/* d_clock is 200 or 600 MHz; which in Q16.32 fixed point ns is: */
	dclk_period = fbnic_is_asic(fbd) ?
		      (((u64)1000000000) << 32) / FBNIC_ASIC_CLOCK_FREQ :
		      (((u64)1000000000) << 32) / FBNIC_FPGA_CLOCK_FREQ;
...
int fbnic_time_start(struct fbnic_net *fbn)
{
	...
	/* assumed that fbnic_ptp_do_aux_work() will never be called if not
	 * scheduled here
	 */
